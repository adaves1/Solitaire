...
namespace XNASolitaire
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        void GameDeactivated(object sender, DeactivatedEventArgs e)
        {
            ...
            {
                isolatedStorageFile.DeleteFile("solitaire.dat");

                using (IsolatedStorageFileStream fileStream
                    = isolatedStorageFile.CreateFile("solitaire.dat"))
                {
                    XmlSerializer xmls = new XmlSerializer(typeof(SolitaireDataList));
                    xmls.Serialize(fileStream, dataList);
                    System.Diagnostics.Debug.WriteLine("Saved to solitaire.dat");
                }
        ...
        void GameActivated(object sender, ActivatedEventArgs e)
        {
            ...
            {
                if (isolatedStorageFile.FileExists("solitaire.dat"))
                {
                    using (IsolatedStorageFileStream fileStream
                        = isolatedStorageFile.OpenFile("solitaire.dat", FileMode.Open))
                    {
                        XmlSerializer xmls = new XmlSerializer(typeof(SolitaireDataList));
                        m_dataList = (SolitaireDataList)xmls.Deserialize(fileStream);
                        System.Diagnostics.Debug.WriteLine("Load from solitaire.dat");
